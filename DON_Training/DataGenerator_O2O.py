...
def project_real_scale(intrinsicCameraReal, objPose, vertex,objectModel,generateObjectDetection = False):

   #print (intrinsicCameraReal.shape, objPose.shape, vertex.shape)

    _3DProjected = np.matmul((objPose[:3]), (vertex.T)).T
    #print (_3DProjected.shape)

    projectedImage = np.matmul((intrinsicCameraReal), np.matmul((objPose[:3]), (vertex.T)))
    scale = 1 / projectedImage[2, :]
    ...
    #print (np.min(projectedImage[2]), np.max(projectedImage[2] ) )
    #print (np.min(_3DProjected[:, 2]),np.max(_3DProjected[:, 2]))

    projectedImage[0, :] -= 380
    return projectedImage.T/ 2.8125 , _3DProjected

def project_real(intrinsicCameraReal, objPose, vertex):
    #print (intrinsicCameraReal.shape, objPose.shape, vertex.shape)
...
class DataGenerator():
    ...
    def get_single_object_within_scene_data(self,pathToScense,matchType,sceneType, augmentationType, renderImage=False,isEvaluate = False,isLoadUnseen=False,isLoadOrig = False):
        ...
        # if (sceneType == MULTI_SAME_REAL or sceneType == MULTI_DIFFERENT_REAL):
        #     imageColor_1, imageColor_2, matches_a, matches_b, masked_non_matches_a, masked_non_matches_b, background_non_matches_a, background_non_matches_b, blind_non_matches_a, blind_non_matches_b \
        #         = self.getMatchPoint(pathObject_1, listObject_1, 0, pathObject_2, listObject_2, 0, matchObject,
        #                              renderImage=renderImage,sceneType=sceneType)
    ...
    def getRealScene(self,isEvaluate=False,isLoadUnseen = False):

        if (isLoadUnseen):
            return self.getRealSceneTest()

        idxObject = 99
        idxScene =  randint(0, nSceneReal - 1)
        pathScene = "GraspNet_train_O2O_real/Object%d/%03d" % (idxObject, idxScene)
        return idxObject, idxScene, pathScene
    ...
    def getSimSceneTest(self): #GraspNet only
        idxObject = 99
        idxScene = randint(0, 19)
        pathScene = "GraspNet_test_O2O_sim2real/Object%d/%03d" % (idxObject, idxScene)
        return idxObject, idxScene, pathScene,False
    def getRealSceneTest(self): #GraspNet only
        idxObject = 99
        idxScene = randint(0, 20)
        pathScene = "GraspNet_test_O2O_real/Object%d/%03d" % (idxObject, idxScene)
        return idxObject, idxScene, pathScene
    ...
    def getScene(self,path,sceneType = 0,isEvaluate = False,isLoadUnseen=False,isLoadOrig = False):  # return 2 scene that have at least 2 mutual objects
        ...
        elif (sceneType == MULTI_DIFFERENT_REAL):
            idxObject_1, idxScene_1, pathScene_1 = self.getRealScene(isLoadUnseen = isLoadUnseen)
            idxObject_2, idxScene_2, pathScene_2 = self.getRealScene(isLoadUnseen = isLoadUnseen)
        elif (sceneType == MULTI_SAME_REAL):
            idxObject_1, idxScene_1, pathScene_1 = self.getRealScene()
            idxObject_2, idxScene_2, pathScene_2 = idxObject_1, idxScene_1, pathScene_1
        elif (sceneType == MULTI_SIM_REAL):
            idxObject_1, idxScene_1, pathScene_1,_ = self.getSimScene(isEvaluate=isEvaluate,isLoadUnseen = isLoadUnseen)
            idxObject_2, idxScene_2, pathScene_2 = self.getRealScene(isLoadUnseen = isLoadUnseen)
        ...
        if (sceneType == SINGLE_TO_MULTI and len (matchObject) ==0):
            ...
            return self.getScene(path, sceneType,isEvaluate = isEvaluate,isLoadUnseen = isLoadUnseen)
        elif ( (sceneType == MULTI_SAME_REAL or sceneType == MULTI_DIFFERENT_REAL) and len(matchObject) < 2):
            return self.getScene(path, sceneType,isLoadUnseen = isLoadUnseen)
        elif ( sceneType == MULTI_SIM_REAL and len(matchObject) < 2):
            return self.getScene(path, sceneType,isEvaluate = isEvaluate,isLoadUnseen = isLoadUnseen)
    ...
    def getImageData_Real(self,pathScene, idx):
        poseCamera = 0
        imageColor = np.array(Image.open(pathScene + "/%06d_rgb.jpg" % (idx)))
        imageColor = cv2.cvtColor(imageColor, cv2.COLOR_BGR2RGB)
        imageColor = imageColor/256
    ...
    def getImageData_Real_Orig(self,pathScene, idx):
        imageColor = np.array(Image.open(pathScene + "/rgb/%04d.png" % (idx)))
        imageColor = cv2.cvtColor(imageColor, cv2.COLOR_BGR2RGB)
        #imageColor = imageColor/256
        depth = cv2.imread(pathScene + "/depth/%04d.png" % (idx), -1)#.astype(float) / 1000.0
        segmentMap =  cv2.imread(pathScene + "/label/%04d.png" % (idx), -1)#.astype(int)


        xmldoc = minidom.parse(pathScene + "annotations/%04d.xml" %(idx))
        itemlist = xmldoc.getElementsByTagName('obj')

        arrObjectPose = np.zeros(((len(itemlist) + 1) * 4, 4))
        #create arrObjectPose
        i = 1
        ...
        return  imageColor, depth, segmentMap, arrObjectPose
    ...
    def getMatchPoint(self,pathObject_1, listObject_1, idx_1, pathObject_2, listObject_2, idx_2, matchObject,nNoneMatchPoint=1000,nMatchPoint = 2000, renderImage=False,sceneType =  0,augmentationType=0,isEvaluate = False,loadITRI = False):
        ...
        if(sceneType < 5):
            ...
            poseCamera_1, imageColor_1, depth_1, mask_1, indexMap_1, segmentMap_1, uvSegment_1, arrObjectPose_1 = self.getImageData_Real( pathObject_1, idx_1)
            poseCamera_2, imageColor_2, depth_2, mask_2, indexMap_2, segmentMap_2, uvSegment_2, arrObjectPose_2 = self.getImageData_Real( pathObject_2, idx_2)
        else: #sim_real
            poseCamera_1, imageColor_1, depth_1, mask_1, indexMap_1, segmentMap_1, uvSegment_1, arrObjectPose_1 = self.getImageData (pathObject_1, idx_1)
            poseCamera_2, imageColor_2, depth_2, mask_2, indexMap_2, segmentMap_2, uvSegment_2, arrObjectPose_2 = self.getImageData_Real( pathObject_2, idx_2)
                ...
                for idxObject_1 in listMatchObject_1:
                    for idxObject_2 in listMatchObject_2:
                        ...
                        if (sceneType < 5):
                            ...
                            uvCoord_1 = project_real(intrinsicCameraReal, objPose_1, selectedModel)
                            uvCoord_2 = project_real(intrinsicCameraReal, objPose_2, selectedModel)
                        else:
